...
$requirements = Join-Path $projectRoot "scripts\python_requirements_hybrid.txt"
& $venvPython -m pip install --retries 8 --timeout 60 --only-binary=:all: -r $requirements
if ($LASTEXITCODE -ne 0) { throw "Failed to install the hybrid Python requirements." }

& $venvPython -c "import sys,pandas,numpy,yaml,h5py,requests,fsspec,gcsfs,zarr,numcodecs; print('Python:',sys.version.split()[0]); print('pandas:',pandas.__version__); print('numpy:',numpy.__version__); print('h5py:',h5py.__version__); print('gcsfs:',gcsfs.__version__); print('zarr:',zarr.__version__); print('Windows-safe hybrid environment ready. Phase 2 uses anonymous public GCS Zarr; Ag3 VCF validation is handled by R/Rsamtools.')"
...
Write-Host ""
Write-Host "Windows-safe Phase2/Ag3 hybrid environment is ready."
Write-Host "Interpreter: $venvPython"
Write-Host "Pointer written to: $pointer"
Write-Host "Next (no network/data download):"
Write-Host "  powershell -ExecutionPolicy Bypass -File scripts\acquire_phase2_discovery.ps1 --dry-run"
