...
* Open the Phase-2 accessibility HDF5 remotely via bounded random-access reads;
  never download the ~1.9 GiB accessibility object as a whole. Only merged windows
  covering the frozen 23-bp target intervals are read and exported.
...
SCHEMA_VERSION = "phase2-public-anonymous-gcs-zarr-v6-x-aware-access-range"
...
def map_phase2_marker_taxon(md: pd.DataFrame) -> pd.Series:
    """Historical M/S marker call retained for audit only."""
    if "m_s" in md.columns:
        m = md["m_s"].astype("string").str.strip().str.upper()
    else:
        m = pd.Series(pd.NA, index=md.index, dtype="string")
    out = pd.Series(pd.NA, index=md.index, dtype="string")
    out.loc[m.eq("M").fillna(False)] = "coluzzii"
    out.loc[m.eq("S").fillna(False)] = "gambiae"
    return out


def map_phase2_taxon(md: pd.DataFrame) -> pd.Series:
    """Authoritative primary taxon from the published Phase-2 population code.

    Release population codes ending in 'gam' denote An. gambiae and those ending
    in 'col' denote An. coluzzii. Mixed/undetermined Phase-2 groups (e.g. GM, GW,
    KE) remain missing and are excluded from the primary species-assigned cohort.
    """
    if "population" not in md.columns:
        return pd.Series(pd.NA, index=md.index, dtype="string")
    pop = md["population"].astype("string").str.strip().str.upper()
    out = pd.Series(pd.NA, index=md.index, dtype="string")
    out.loc[pop.str.endswith("GAM", na=False)] = "gambiae"
    out.loc[pop.str.endswith("COL", na=False)] = "coluzzii"
    return out
...
def prepare_metadata(md: pd.DataFrame, primary_taxa: set[str]) -> pd.DataFrame:
    ...
    out["marker_taxon_m_s"] = map_phase2_marker_taxon(out)
...
def read_cached_gt_header_samples(path: Path) -> np.ndarray:
    """Read only the header of a previously completed target-genotype cache."""
    with path.open("r", encoding="utf-8", newline="") as f:
        row = next(csv.reader(f, delimiter="\t"), None)
    if row is None or row[:len(FIXED_GT_COLUMNS)] != FIXED_GT_COLUMNS:
        fail(f"Malformed cached phased-target TSV header: {path}")
    samples = np.asarray(row[len(FIXED_GT_COLUMNS):], dtype=object)
    if len(set(samples.astype(str).tolist())) != len(samples):
        fail(f"Duplicate sample columns in cached phased-target TSV: {path}")
    return samples


def reusable_contig_cache(out: Path, done_path: Path, contig: str):
    """Return (done_json, sample_names) when a previous contig cache is intact."""
    if not out.exists() or not done_path.exists():
        return None
    try:
        done = json.loads(done_path.read_text(encoding="utf-8"))
        if str(done.get("contig")) != contig:
            return None
        if int(done.get("bytes", -1)) != out.stat().st_size:
            return None
        if str(done.get("sha256", "")).lower() != sha256_file(out).lower():
            return None
        samples = read_cached_gt_header_samples(out)
        if int(done.get("n_samples", -1)) != len(samples):
            return None
        return done, samples
    except Exception:
        return None
...
class RangeReadBudget:
    """Read-only file-like guard for h5py remote random access.

    The underlying GCS file is opened with cache_type="none", so each h5py read
    is served as an explicit byte-range request rather than a whole-object fetch.
    A cumulative byte budget prevents an unexpected HDF5 access pattern from
    silently transferring a large fraction of the remote object.
    """
    ...
    def _check(self, n: int) -> None:
        if n is None or int(n) < 0:
            raise OSError("Unbounded read refused for remote accessibility HDF5.")
        if self.bytes_read + int(n) > self.max_bytes:
            raise OSError(
                f"Remote accessibility read budget would be exceeded: "
                f"{(self.bytes_read + int(n)) / 1024**2:.1f} MiB requested > "
                f"{self.max_bytes / 1024**2:.1f} MiB ceiling."
            )
    ...
    def write(self, *_args, **_kwargs):
        raise io.UnsupportedOperation("remote accessibility HDF5 is read-only")
...
def build_accessibility_rows(h5, qsites: pd.DataFrame, acc_windows: list[Interval]) -> pd.DataFrame:
    """Read merged accessibility windows and derive target-level summaries."""
    rows = []
    windows_by_contig = {c: [w for w in acc_windows if w.contig == c] for c in CANONICAL}
    ...
    for contig in CANONICAL:
        ...
        for w in windows_by_contig.get(contig, []):
            if w.start < 1 or w.end > arr.shape[0]:
                fail(f"Accessibility window out of bounds: {w.region}; contig length={arr.shape[0]:,}")
            block = np.asarray(arr[w.start - 1:w.end], dtype=bool)
            if len(block) != w.span:
                fail(f"Accessibility range length mismatch for {w.region}: expected {w.span}, got {len(block)}")

            inside = ss[(ss.genomic_start.astype(int) >= w.start) & (ss.genomic_end.astype(int) <= w.end)]
            for _, site in inside.iterrows():
                sid = str(site.population_site_id)
                if sid in vectors:
                    fail(f"Accessibility site assigned to more than one merged window: {sid}")
                st, en = int(site.genomic_start), int(site.genomic_end)
                a = st - w.start
                b = en - w.start + 1
                v = np.asarray(block[a:b], dtype=bool)
                if len(v) != 23:
                    fail(f"Expected 23 accessibility bases for {sid}; got {len(v)}")
                vectors[sid] = v

        expected_ids = set(ss.population_site_id.astype(str))
        missing = sorted(expected_ids - set(vectors))
        if missing:
            fail(f"{contig}: {len(missing)} targets were not covered by accessibility request windows; first={missing[:5]}")

        for _, site in ss.iterrows():
            sid = str(site.population_site_id)
            v = vectors[sid]
            pam = np.zeros(23, dtype=bool)
            strand = str(site.guide_genomic_strand)
            if strand == "+":
                pam[-3:] = True
            elif strand == "-":
                pam[:3] = True
            else:
                fail(f"Invalid strand for {sid}")
            rows.append({
                ...
                "pam_accessibility_fraction": float(v[pam].mean()),
                "protospacer_accessibility_fraction": float(v[~pam].mean()),
            })

    out = pd.DataFrame(rows)
    if len(out) != 515 or out.population_site_id.duplicated().any():
        fail(
            f"Accessibility handoff guard failed; rows={len(out)}, "
            f"duplicates={out.population_site_id.duplicated().sum()}"
        )
    return out
...
def main() -> int:
    ...
    ap.add_argument("--probe-only", action="store_true", help="Verify anonymous public GCS/Zarr discovery and report source metadata without downloading genotype chunks.")
    ...
    log("Safety design: phased discovery uses anonymous public legacy GCS chunked Zarr; no chromosome VCF is downloaded")
    ...
    if args.probe_only:
        probe = {
            ...
            "top_level_keys": list(group.group_keys())[:50] if hasattr(group, "group_keys") else [],
            "n_primary_metadata_samples": int(len(primary_md)),
            "status": "anonymous Phase-2 Zarr probe succeeded",
        }
        atomic_json(out_dir / "phase2_public_zarr_probe.json", probe)
        log(f"PROBE SUCCESS: anonymous phased Zarr opened at {zarr_source}")
        log("No phased genotype chunks or accessibility object were downloaded in probe-only mode")
        return 0

    contig_done = []
    selected_by_contig: dict[str, np.ndarray] = {}
    want = primary_md.sample_id.astype(str).tolist()
    for contig in CANONICAL:
        ss = qsites[qsites.genomic_seqid.astype(str) == contig].copy()
        if ss.empty: continue
        out = out_dir / "haplotypes" / f"{contig}.phased_targets.tsv"
        done_path = out_dir / "haplotypes" / f"{contig}.done.json"

        cached = reusable_contig_cache(out, done_path, contig)
        if cached is not None:
            done, selected_names = cached
            selected_by_contig[contig] = selected_names.astype(object)
            contig_done.append(done)
            log(f"{contig}: reusing verified cache with {int(done.get('n_variants', 0)):,} target-overlap SNPs for {len(selected_names):,} samples")
            continue
        ...
        selected_by_contig[contig] = selected_names
        n_missing = len(want) - len(selected_names)
        if n_missing:
            log(f"{contig}: phased sample membership differs from the full primary cohort; {len(selected_names):,} present, {n_missing:,} absent. Missing contig/sample combinations will remain NA downstream.")
        ...
        done = {"contig":contig,"n_target_sites":int(len(ss)),"n_variants":int(len(pos)),"n_samples":int(len(selected_names)),"n_primary_samples_absent":int(n_missing),"bytes":int(out.stat().st_size),"sha256":sha256_file(out)}
        atomic_json(done_path, done)
        contig_done.append(done)
        log(f"{contig}: saved {len(pos):,} phased target-overlap SNPs for {len(selected_names):,} samples")

    if not selected_by_contig: fail("No phased samples acquired.")
    present_union = set().union(*[set(x.astype(str).tolist()) for x in selected_by_contig.values()])
    present_common = set(want)
    for x in selected_by_contig.values():
        present_common &= set(x.astype(str).tolist())
    selected_union = np.asarray([s for s in want if s in present_union], dtype=object)
    selected_common = np.asarray([s for s in want if s in present_common], dtype=object)
    phase_md = md.set_index("sample_id").loc[selected_union.tolist()].reset_index()
    atomic_df(phase_md, out_dir / "metadata" / "phased_sample_metadata.csv")

    membership = pd.DataFrame({"sample_id": want})
    for contig in CANONICAL:
        present = set(selected_by_contig.get(contig, np.asarray([], dtype=object)).astype(str).tolist())
        membership[f"present_{contig}"] = membership["sample_id"].isin(present)
    membership["n_contigs_present"] = membership[[f"present_{c}" for c in CANONICAL]].sum(axis=1).astype(int)
    atomic_df(membership, out_dir / "metadata" / "phased_sample_membership_by_contig.csv")
    log(f"Phase-2 phased sample union={len(selected_union):,}; present on all queried contigs={len(selected_common):,}")

    # Accessibility: remote HDF5 random-access only. The ~1.9 GiB object is never
    # downloaded as a whole. h5py reads through a seekable anonymous GCS file-like
    # object, with cache_type="none" and an explicit cumulative read budget.
    if args.manual_haplotype_zarr:
        log("Manual Zarr mode still uses the public Phase-2 accessibility HDF5 via anonymous GCS range reads.")
        bucket = str(acq.get("public_gcs_bucket", "ag1000g-release"))
        prefix = str(acq.get("public_gcs_prefix", "phase2.AR1"))
        gfs = gcsfs.GCSFileSystem(token="anon")
    remote_acc = find_accessibility_object(gfs, bucket, prefix)
    acc_info = gfs.info(remote_acc)
    acc_size = int(acc_info.get("size", -1))
    if acc_size < 0:
        fail(f"Could not determine remote accessibility object size for gs://{remote_acc}.")
    acc_object_mb = acc_size / 1024**2
    read_budget_mb = float(acq.get("max_accessibility_remote_read_mb", 512))
    block_kib = int(acq.get("accessibility_remote_block_kib", 256))
    if block_kib < 64 or block_kib > 4096:
        fail(f"accessibility_remote_block_kib={block_kib} is outside the safety range 64-4096 KiB.")
    log(
        f"Accessibility source is {acc_object_mb:.1f} MiB remotely; whole-object download is disabled. "
        f"Reading only {len(acc_win)} merged target window(s) with a {read_budget_mb:.1f} MiB cumulative read ceiling."
    )
    ...
    try:
        with gfs.open(
            ...
            guarded = RangeReadBudget(raw_acc, int(read_budget_mb * 1024**2))
            with h5py.File(guarded, "r") as h5:
                acc = build_accessibility_rows(h5, qsites, acc_win)
    except OSError as exc:
        fail(
            "Remote accessibility HDF5 range access failed or exceeded its transfer budget. "
            f"No whole-file fallback was attempted. Details: {exc}"
        )

    acc_bytes_read = int(guarded.bytes_read if guarded is not None else 0)
    acc_read_calls = int(guarded.read_calls if guarded is not None else 0)
    log(
        f"Accessibility extraction complete: 515 targets; h5py requested "
        f"{acc_bytes_read / 1024**2:.1f} MiB across {acc_read_calls:,} bounded range read(s)."
    )
    atomic_df(acc, out_dir / "accessibility" / "target_accessibility.tsv", sep="\t")

    qc_df = pd.DataFrame(contig_done)
    atomic_df(qc_df, out_dir / "acquisition_qc.tsv", sep="\t")
    ...
    manifest = {
        ...
        "accessibility_access_mode": "remote_hdf5_bounded_random_access",
        "accessibility_remote_object_mib": round(acc_object_mb, 3),
        "accessibility_h5py_bytes_requested": acc_bytes_read,
        "accessibility_h5py_read_calls": acc_read_calls,
        "whole_accessibility_object_downloaded": False,
        "n_frozen_targets": int(len(sites)),
        "n_canonical_targets": int(len(qsites)),
        "n_phased_samples": int(len(selected_union)),
        "n_phased_samples_all_contigs": int(len(selected_common)),
        "contigs": contig_done,
